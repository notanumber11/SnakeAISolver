...
basic_dnn_solver = solvers.basic_dnn.basic_dnn_solver.BasicDnnSolver()
basic_solver = BasicSolver()
all_solvers = [random_solver, basic_solver, dfs_solver, basic_dnn_solver, hamilton_solver]
...
def get_all_game_types():
    return [_get_default_game(solver) for solver in all_solvers]


def _get_default_game(solver):
...
def get_default_games(solver, number) -> List[Game]:
    ...
    for i in range(number):
        games.append(_get_default_game(solver))
    return games


def get_random_game(solver, board_size, snake_size=None):
    if not snake_size:
        snake_size = random.randint(2, snake_size)
...
@timeit
def get_random_games(solver, number_of_games, board_size=6, snake_size=5) -> List[Game]:
    games = []
    for i in range(number_of_games):
        games.append(get_random_game(solver, board_size, snake_size))
    return games
