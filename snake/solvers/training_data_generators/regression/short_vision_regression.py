...
class ShortVisionRegression(TrainingDataGenerator):

    def generate_random_training_data(self, grid_size, snake_size, samples: int = 100):
        """
        Generate number of samples for training_data_generators with the following format:
        ...
        while not enough_samples:
            _game = game.game_provider.GameProvider.get_random_game(solver, grid_size, snake_size)
            for i in range(1, len(_game.game_statuses)):
                ...
                angle = solvers.training_data_generators.data_utils.normalize_rad_angle(
                ...
                reward = self.get_reward(current, next_)
                row = [up, down, left, right, up_available, down_available, left_available, right_available, angle,
                       reward]
    ...
    def _is_apple_closer(self, current: GameStatus, next: GameStatus):
    ...
    def get_reward(self, current, next_):
        reward_values = {
            "eating": 0.7,
            "closer": 0.1,
            "further": -0.2,
        ...
        if not next_.is_valid_game():
            ...
            if self._is_apple_closer(current, next_):
                reward = reward_values["closer"]
            elif not self._is_apple_closer(current, next_):
